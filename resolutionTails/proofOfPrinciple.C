...
void proofOfPrinciple(unsigned int nEntries = 500000) {
  ...
  TLegend* leg = util::LabelFactory::createLegendCol(3,0.35);
  leg->AddEntry(hResp.front(),"f_{resp} #times "+util::toTString(varTailResp.front(),1),"L");
  leg->AddEntry(hResp.at(nVar),"f_{resp} #times 1.0","L");
  leg->AddEntry(hResp.back(),"f_{resp} #times "+util::toTString(varTailResp.back(),1),"L");
  ...
  cFvsF->cd();
  TH1* hFrameFAsymVsFResp = new TH1D("hFrameFAsymVsFResp",";Scale of f_{resp};Scale of f_{asym}",1000,0.4,1.6);
  ...
  cRelVar->cd();
  TH1* hFrameRelScale = new TH1D("hFrameRelScale",";Scale of f_{resp};Scale of f_{resp} / Scale of f_{asym}",1000,0.4,1.6);
