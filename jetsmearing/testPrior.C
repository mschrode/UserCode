...
//!  greater than \p Parameter::ptDijetMin. Additionally, the 
...
//!  with dijet pt greater than \p Parameters::ptDijetMin.
//------------------------------------------------------------------
TF1 * createPDF(const TString& name, const Parameter& par, bool cut) {
  TF1 * pdf = new TF1(name,pdfTruth,par.pdfMin,par.pdfMax,8);
  ...
  pdf->SetParameter(6,par.expo);
  pdf->SetParameter(7,par.ptGenMin);
...
double pdfTruth(double * x, double * par) {
  ...
  if( par[0] == 1. ) {
    ...
    weight = fResolution.Integral(ptDijetMin,pdfMax);
  }
